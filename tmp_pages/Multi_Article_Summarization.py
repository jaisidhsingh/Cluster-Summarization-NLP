...
st.markdown("Extractive Summarization for Multiple Articles 😊")
...
if START and not COMPLETED:
	...
	if num_words <= summarizer_token_limit:
		with tmp_out.container():
			st.write("Input token count (",num_words,") <= token limit (",summarizer_token_limit,"), skipping optimization ...")
		...
		with tmp_out.container():
			st.write("Input token count (",num_words,") > token limit (",summarizer_token_limit,"), optimizing ...")
